...
export default function BulkLinkProductsPage() {
  // State
  const [selectedBaseProductId, setSelectedBaseProductId] = useState<number | null>(null);
  ...
  const { data: linkableProductsData, refetch: refetchLinkable } = useGetProductsQuery({
  ...
  const handleUpdateMultiplier = (productId: number, multiplier: string) => {
    setSelectedProducts((prev) =>
      prev.map((sp) =>
        sp.product.id === productId ? { ...sp, quantity_multiplier: multiplier } : sp
      )
    );
  };
  ...
  const handleBulkLink = async () => {
    ...
    if (failCount === 0) {
      toast.success(`Successfully linked ${successCount} product(s) to base product`);
      setSelectedProducts([]);
      refetchLinkable();
    } else {
      ...
      <DashboardLayout>
        <div className="mb-6">
          ...
          <p className="mt-2 text-gray-600">
            Link multiple products to a base product at once. Only products with 0 stock can be linked.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6">
            <h2 className="mb-4 text-lg font-semibold">1. Select Base Product</h2>
            <SearchableSelect
              label="Base Product"
            ...
            {selectedBaseProduct && (
              <div className="mt-4 rounded-lg bg-blue-50 p-4">
                <h3 className="font-semibold text-blue-900">{selectedBaseProduct.name}</h3>
                <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                  <div>
                    <span className="text-gray-600">SKU:</span>{" "}
                    <span className="font-medium">{selectedBaseProduct.sku || "N/A"}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Stock:</span>{" "}
                    <span className="font-medium">{selectedBaseProduct.stock_quantity}</span>
                  </div>
                  <div>
            ...
            <div className="mt-6">
              <h3 className="mb-2 text-sm font-semibold text-gray-700">Default Multiplier</h3>
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6">
            <h2 className="mb-4 text-lg font-semibold">2. Select Products to Link</h2>
            ...
            <div className="mb-4 max-h-48 overflow-y-auto rounded border border-gray-200">
              {linkableProducts.length === 0 ? (
                <p className="p-4 text-center text-sm text-gray-500">
                  No eligible products found. Products must have 0 stock and not be already linked.
                </p>
              ) : (
            ...
            <div className="max-h-64 overflow-y-auto rounded border border-green-200 bg-green-50">
                  ...
                  {selectedProducts.map((sp) => (
                    <div key={sp.product.id} className="flex items-center gap-3 p-3">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleUpdateMultiplier(sp.product.id, e.target.value)
                        }
                        className="w-24"
                        placeholder="Mult."
                      />
        ...
        <div className="mt-6 flex items-center justify-between rounded-lg bg-gray-100 p-4">
          <div className="text-sm text-gray-600">
            {selectedBaseProductId ? (
              <>
                Linking <span className="font-semibold">{selectedProducts.length}</span> product(s)
                to <span className="font-semibold">{selectedBaseProduct?.name}</span>
              </>
            ) : (
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={!selectedBaseProductId || selectedProducts.length === 0 || isLinking}
            >
              {isLinking
